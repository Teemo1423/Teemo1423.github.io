(()=>{
  ...
  function injectStyles(){
    ...
    const s=document.createElement('style');s.id='churchNavHierarchy';s.textContent=`
      ...
      @media(max-width:900px){
        ...
        .mobile-bottom-nav a .mb-icon{font-size:22px;line-height:1.1;font-family:Arial,sans-serif}.mobile-bottom-nav a.active{color:var(--green,#2f6b4f);background:#eef5f1}.mobile-bottom-nav a:active{background:#e7f1eb;transform:scale(.97)}
      }
  ...
  function run(){injectStyles();document.querySelectorAll('.mobile-menu-toggle,.nav-backdrop').forEach(x=>x.remove());document.querySelectorAll('.navlinks').forEach(buildDesktop);injectBottomNav()}
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',run,{once:true});else run();
})();

(()=>{if(document.querySelector('script[data-site-image-adjust]'))return;const s=document.createElement('script');s.dataset.siteImageAdjust='1';s.src='/js/site-image-adjust.js?v=imageadjust2';document.head.appendChild(s)})();
