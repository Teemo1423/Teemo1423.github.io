(()=>{
  ...
  function injectStyles(){
    const old=document.getElementById('churchNavHierarchy');if(old)old.remove();
    const s=document.createElement('style');s.id='churchNavHierarchy';s.textContent=`
      ...
      @media(max-width:900px){
        ...
        .site-header .navlinks>a{display:flex!important;align-items:center!important;min-height:54px!important;padding:0 22px!important;font-size:17px!important;font-weight:800!important;border-bottom:1px solid var(--line,#e4e4df)!important}
        .site-header .navitem{display:block!important;border-bottom:1px solid var(--line,#e4e4df)!important}
        .site-header .navitem>a{display:flex!important;align-items:center!important;min-height:54px!important;padding:0 22px!important;font-size:17px!important;border-radius:0!important}
        ...
        .nav-backdrop{display:block!important;position:fixed!important;inset:0!important;background:rgba(12,25,20,.34)!important;z-index:1399!important;opacity:0!important;visibility:hidden!important;pointer-events:none!important;transition:opacity .25s ease,visibility .25s ease!important}.nav-backdrop.show{opacity:1!important;visibility:visible!important;pointer-events:auto!important}
      }
    `;document.head.appendChild(s)
  }
  function build(nav){if(!nav)return;const path=location.pathname||'/';nav.innerHTML='';const home=document.createElement('a');home.href='/';home.textContent='홈';if(path==='/'||path.endsWith('/index.html'))home.classList.add('active');nav.appendChild(home);menu.forEach(group=>{const item=document.createElement('div');item.className='navitem';const a=document.createElement('a');a.href=group.href;a.textContent=group.label;const all=[group.href,...group.items.map(x=>x[1])];if(all.some(h=>path===h||path.endsWith(h.replace(/^\//,''))))a.classList.add('active');const dd=document.createElement('div');dd.className='dropdown';group.items.forEach(([label,href])=>{const x=document.createElement('a');x.href=href;x.textContent=label;if(path===href||path.endsWith(href.replace(/^\//,'')))x.classList.add('active');dd.appendChild(x)});item.append(a,dd);nav.appendChild(item)})}
  function enhance(nav){if(!nav||nav.dataset.mobileStable==='2')return;nav.dataset.mobileStable='2';build(nav);const wrap=nav.closest('.nav');if(!wrap)return;let btn=wrap.querySelector('.mobile-menu-toggle');if(!btn){btn=document.createElement('button');btn.type='button';btn.className='mobile-menu-toggle';btn.setAttribute('aria-label','메뉴 열기');btn.setAttribute('aria-expanded','false');btn.innerHTML='<span></span><span></span><span></span>';const brand=wrap.querySelector('.brand');brand?brand.insertAdjacentElement('afterend',btn):wrap.insertBefore(btn,nav)}let backdrop=document.querySelector('.nav-backdrop');if(!backdrop){backdrop=document.createElement('div');backdrop.className='nav-backdrop';document.body.appendChild(backdrop)}
    ...
    const toggle=e=>{e.preventDefault();e.stopPropagation();nav.classList.contains('mobile-open')?close():open()};
    if(window.PointerEvent)btn.addEventListener('pointerup',toggle,{passive:false});else btn.addEventListener('click',toggle,false);
    backdrop.addEventListener('click',close,false);nav.addEventListener('click',e=>{if(e.target.closest('a'))close()},false);window.addEventListener('resize',()=>{if(innerWidth>900)close()},{passive:true});document.addEventListener('keydown',e=>{if(e.key==='Escape')close()});
  }
  injectStyles();const run=()=>document.querySelectorAll('.navlinks').forEach(enhance);if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',run,{once:true});else run();new MutationObserver(run).observe(document.documentElement,{childList:true,subtree:true});
})();